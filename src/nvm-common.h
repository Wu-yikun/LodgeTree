 ...
 */
static inline void clwb(void *addr) { asm volatile("clwb %0" : : "m"(*((char *)addr))); }
...
static inline void NVMFLUSH_STAT_print() { printf("num_clwb=%lld, num_sfence=%lld\n", num_clwb, num_sfence); }
...
class NvmLog_Log {
 ...
 public:
  ...
  void printLog() {
    ...
    for (char *p = nl_log_area_; p < nl_log_area_end_; p += 16) {
      ...
      for (int ii = 0; ii < 16; ii++) printf(" %02x", (unsigned char)p[ii]);
      printf("\n");
    }
  }
  ...
  void printLogWritePos() {
    printf("write version_:%02x, offset_:%3d, next_ptr_:%d, nextline_ptr_:%d\n", nl_log_wr_.version_,
           nl_log_wr_.offset_, (int)(nl_log_wr_.next_ptr_ - (nl_log_wr_.nextline_ptr_ - CACHE_LINE_SIZE)),
           (int)(nl_log_wr_.nextline_ptr_ - nl_log_area_));
  }

  // for debugging
  void printLogReadPos(nl_log_pointer_t *pos) {
    printf("read version_:%02x, offset_:%3d, next_ptr_:%d, nextline_ptr_:%d\n", pos->version_, pos->offset_,
           (int)(pos->next_ptr_ - nl_log_area_), (int)(pos->nextline_ptr_ - nl_log_area_));
  }

  // for debugging
  unsigned long long getLogOffset(void *p) { return (unsigned long long)((char *)p - nl_log_area_); }

 private:
  char computeCheckSum(char *line) {
    unsigned long long *p = (unsigned long long *)line;
    unsigned long long v = (p[0] & 0xffffffffffffff00ULL) + p[1] + p[2] + p[3] + p[4] + p[5] + p[6] + p[7];
    v += v >> 32;
    v += v >> 16;
    v += v >> 8;
    return (char)v;
 ...
 public:
   ...
   */
  void prepareForRead(nl_log_pointer_t *pos) { flushLog(); }

  /**
   * obtain the end position of the log
   */
  char *getLogWriteEndPtr(void) { return nl_log_wr_.next_ptr_; }
 ...
 public:
  ...
  NlVolatileRedoRec *getRedoRec() {
    if (nl_num_redo_rec_ == nl_cap_redo_rec_) {
      nl_cap_redo_rec_ *= 2;
      nl_redo_rec_ = (NlVolatileRedoRec *)realloc(nl_redo_rec_, sizeof(NlVolatileRedoRec) * nl_cap_redo_rec_);
      if (nl_redo_rec_ == NULL) {
        perror("realloc");
        exit(1);
      }
  ...
  void commitMiniTransaction() {
    ...
    // actual deletions
    if (nl_num_node_to_del_ > 0) {
      for (int i = 0; i < nl_num_node_to_del_; i++) nvmpool_free_node(nl_node_to_del_[i]);
    }
  }
      ...
      switch ((unsigned char)(pos.next_ptr_[0])) {
        ...
        case NL_WRITE1B:
          ...
          printf("WRITE1B addr=%p old_value=%02x new_value=%02x\n", *((void **)(p + 1)),
                 *((unsigned char *)(p + 1 + 8)), *((unsigned char *)(p + 1 + 8 + 1)));
          break;
        case NL_WRITE2B:
          nl_logbuf_.readLog(&pos, p, 1 + 8 + 2 + 2 + 1);
          printf("WRITE2B addr=%p old_value=%04x new_value=%04x\n", *((void **)(p + 1)),
                 *((unsigned short *)(p + 1 + 8)), *((unsigned short *)(p + 1 + 8 + 2)));
          break;
        case NL_WRITE4B:
        ...
        case NL_WRITE8B:
          ...
          printf("WRITE8B addr=%p old_value=%016llx new_value=%016llx\n", *((void **)(p + 1)),
                 *((unsigned long long *)(p + 1 + 8)), *((unsigned long long *)(p + 1 + 8 + 8)));
          break;
        case NL_WRITEVCHAR:  // 1 + 8 + 4 + len + len + 4 + 1
          nl_logbuf_.readLog(&pos, p, 1 + 8 + 4);
          len = *((int *)(p + 1 + 8));
 ...
 public:
  ...
  void delNode(void *p) {
    if (nl_num_node_to_del_ == nl_cap_node_to_del_) {
      ...
      nl_node_to_del_ = (void **)realloc(nl_node_to_del_, sizeof(void *) * nl_cap_node_to_del_);
      if (nl_node_to_del_ == NULL) {
        perror("realloc");
        exit(1);
      }
